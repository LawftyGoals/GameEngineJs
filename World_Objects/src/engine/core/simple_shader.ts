...
export class SimpleShader {
  ...
  constructor(
    ...
    this.mVertexShader = loadAndCompileShader(
      this.gl,
      vertexShaderId,
    ...
    if (
      !this.gl.getProgramParameter(this.mCompiledShader, this.gl.LINK_STATUS)
    ) {
      throw new Error("Error linking shader");
    }

    this.mVertexPositionRef = this.gl.getAttribLocation(
...
function loadAndCompileShader(
  ...
  if (!compiledShader) {
    throw new Error(
      `shader of type ${shaderType} is not being created in loadAndCompileShader`
    );
  }
